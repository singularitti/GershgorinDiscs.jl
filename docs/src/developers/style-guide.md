...
!!! info
    The style guide is always a work in progress, and not all GershgorinDiscs code
    follows the rules. When modifying GershgorinDiscs, please fix the style violations
...
GershgorinDiscs uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/GershgorinDiscs.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the GershgorinDiscs directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to GershgorinDiscs have
