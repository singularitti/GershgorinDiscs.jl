...
[GershgorinDiscs](https://github.com/singularitti/GershgorinDiscs.jl).
...
version of GershgorinDiscs, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/GershgorinDiscs.jl"
```
...
our [GitHub repository](https://github.com/singularitti/GershgorinDiscs.jl)
for new releases.
Once we release a new version, you can update GershgorinDiscs by typing

```@repl
using Pkg
Pkg.update("GershgorinDiscs")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("GershgorinDiscs")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall GershgorinDiscs.
